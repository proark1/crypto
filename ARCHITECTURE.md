...
| Backtester: runner, pessimistic fill simulator, golden test (§5) | **Done** — walk-forward splitting feeds the parameter sweeps (§12.5); account-level multi-symbol runner (one strategy per symbol, one shared book/risk manager, deterministic candle interleave) exercises exposure ceilings and balance contention no single-symbol backtest can show |
