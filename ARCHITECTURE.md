...
| News pipeline, regime gates, signal fusion (§5.2, §5.3) | **Partial** — BTC regime gate done (ADX trend/range + drawdown risk-off; entries gated in every mode, exits never; verdicts journaled as `gated` decisions); news pipeline done defensively (CryptoPanic polling + keyword classifier, negative-news coin flags block entries + alert the human, env-configured FOMC/CPI/unlock no-entry windows); F&G/dominance inputs, confirmation filters, broad-flow→risk-off coupling, and automated calendar ingestion missing |
