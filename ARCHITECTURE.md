...
| News pipeline, regime gates, signal fusion (§5.2, §5.3) | **Partial** — BTC regime gate done (ADX trend/range + drawdown risk-off on the reference market; entries gated in every mode, exits never; gate verdicts journaled as `gated` decisions); F&G/dominance inputs, confirmation filters, and the news pipeline missing |
