...
| `TRADEBOT_HISTORY_BACKFILL_DAYS` | no | `0` — set e.g. `1095` to fetch 3 years of 1m history (free, public REST) the first time a coin has no stored candles; expect the initial crawl to take ~30–60 min per coin-year and ~0.5 GB of Postgres per coin-year |
